...
const domainConfig: Record<string, { label: string; color: string }> = {
  ...
  "Academic": { label: "Academic", color: "bg-amber-100 text-amber-700" },
};
...
export function PortfolioCard({ item, index }: PortfolioCardProps) {
  ...
  return (
    <motion.div
      className={`bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-slate-200 flex flex-col h-full ${item.domain !== "Academic" ? "cursor-pointer" : ""}`}
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: index * 0.1 }}
      onClick={() => item.domain !== "Academic" && (window.location.hash = `project-${item.id}`)}
    >
