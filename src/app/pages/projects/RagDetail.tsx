      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-sm text-slate-500">2025.06 - 2025.07</span>
            <span className="px-3 py-1 rounded-md text-sm font-medium bg-slate-100 text-slate-600">(주)사운드마인드</span>
          </div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">기업 문서 RAG 질의응답 시스템</h1>
        ...
        <div id="rag-2">
          <Section title="3. DocumentManagement Graph" delay={0.2}>
            ...
            <div className="bg-slate-50 rounded-lg p-4 my-4">
              <h4 className="text-sm font-semibold text-slate-800 mb-3">Add 경로 — 6개 노드</h4>
              ...
              <div className="mt-4 pt-3 border-t border-slate-200">
                <h4 className="text-sm font-semibold text-slate-800 mb-2">Delete 경로</h4>
                <p className="text-sm text-slate-600">routing_node → delete_vector_db_node → END</p>
              </div>
            </div>
          ...
          <Section title="4. ChatBot Graph" delay={0.25}>
            ...
            <div className="grid md:grid-cols-2 gap-4 my-4">
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                <h4 className="text-sm font-semibold text-slate-800 mb-2">QA 경로</h4>
                <div className="space-y-2">
                  <p className="text-xs text-slate-600">1. TaskRouting → "QA"</p>
                  <p className="text-xs text-slate-600">2. VectorCheckNode — VectorDB 존재 확인</p>
                  <p className="text-xs text-slate-600">3-a. 있으면 → GetRetrievalNode (Top-3 검색) → GetAnswerNode (스트리밍 응답)</p>
                  <p className="text-xs text-slate-600">3-b. 없으면 → EmptyVectorResponseNode ("문서를 업로드해주세요")</p>
                </div>
              </div>
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                <h4 className="text-sm font-semibold text-slate-800 mb-2">Summary 경로</h4>
                <div className="space-y-2">
                  <p className="text-xs text-slate-600">1. TaskRouting → "Summary"</p>
                  <p className="text-xs text-slate-600">2. DocumentSummaryNode</p>
                  <p className="text-xs text-slate-600">3. Map(페이지별 요약) → Reduce(재귀적 축약)</p>
                  <p className="text-xs text-slate-600">4. figure/chart 카테고리 자동 필터링</p>
                </div>
              </div>
            </div>
        ...
        <div id="rag-4">
          <Section title="5. 문서 파싱 및 구조 보존" delay={0.3}>
            ...
            <div className="grid md:grid-cols-2 gap-4 mb-4">
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                <h4 className="text-sm font-semibold text-slate-800 mb-2">Before — 일반 텍스트 추출</h4>
                <ul className="space-y-1">
                  <li className="text-xs text-slate-600">• 표 구조 손실</li>
                  <li className="text-xs text-slate-600">• 제목 계층 무시</li>
                  <li className="text-xs text-slate-600">• 이미지 참조 누락</li>
                  <li className="text-xs text-slate-600">• 청킹 시 의미 단위 깨짐</li>
                </ul>
              </div>
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                <h4 className="text-sm font-semibold text-slate-800 mb-2">After — Upstage Document Parse</h4>
                <ul className="space-y-1">
                  <li className="text-xs text-slate-600">• HTML/Markdown/Text 3형식 출력</li>
                  <li className="text-xs text-slate-600">• 제목/목차/표/리스트 구조 보존</li>
                  <li className="text-xs text-slate-600">• figure/chart base64 → PNG 저장</li>
                  <li className="text-xs text-slate-600">• 페이지별 Document + metadata</li>
                </ul>
              </div>
            </div>
        ...
        <div id="rag-6">
          <Section title="7. 로컬 임베딩 + 멀티 유저 데이터 격리" delay={0.4}>
            ...
            <div className="bg-slate-50 rounded-lg p-4 mb-4">
              <h4 className="text-sm font-semibold text-slate-800 mb-2">유저별 디렉토리 구조</h4>
